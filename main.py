...
def load_csv(data_dir, prefix):
    csv_files = sorted(
        f for f in os.listdir(data_dir) if f.endswith(".csv") and f.startswith(prefix)
    )
    if not csv_files:
        return None
    with open(os.path.join(data_dir, csv_files[-1]), newline="", encoding="utf-8") as f:
